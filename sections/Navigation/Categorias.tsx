...
export default function Section({ 
    ...
    <section className="grid gap-8 lg:gap-20 pb-[65px] bg-white">
        <div>
             {/* Título para mobile */}
            <p className="pt-[52px] md:hidden pl-[17px] pb-[42px] text-sans font-semibold text-sacramentoState text-[22px] leading-tight-25 tracking-tight-2 text-left">{titleForMobile}</p>
            {/* Título para desktop */}
            <p className="pt-[144px] px-[240px] hidden md:block text-sans font-semibold text-sacramentoState text-5xl font-semibold leading-tight-55 tracking-tight-2 text-center">{titleForDesktop}</p>
                ...
                {tags?.map(tag => (
                  <div key={tag.name} className="w-fit sm:w-1/2 lg:w-fit rounded-[58px] inline-flex px-4 py-3 bg-mintCream text-sans font-semibold text-primary items-center">
                    <div className="inline-flex items-center">
                      <Image className="w-4 h-4" src={tag.icon || ""} alt={tag.name} width={10.67} />
                      <a href={tag.url} className="text-md md:text-lg pl-2 pr-[1.5px] text-sans font-semibold leading-tight-18">
                        {tag.name}
                      </a>
                    </div>
                  </div>
