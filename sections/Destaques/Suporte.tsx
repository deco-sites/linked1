...
// Implemente a seção
function setClassTitle(value: string) {
  switch (value) {
    case "Primary":
    ...
    <section className={`pl-4 pr-4 ${setClass(background)}`}>
      <div className="container mx-auto text-center pt-[56px] pb-[120px]">
        <div className=" block justify-between items-center">
          ...
          <h2
            className={`pt-[43px] pb-[32px] text-[28px] md:text-5xl font-sans ${
              setClassTitle(background)
            } font-normal text-center leading-tight-32 tracking-tight-2`}
          >
            {subTitle}
          </h2>
        </div>
        ...
        <div
          className={`grid gap-[1px] ${setClassBorder(background)} ${
            content.length === 1
              ...
              : content.length === 4 || content.length === 8 ||
                  content.length >= 8
          ...
          {content.slice(0, 8).map((item, index) => {
            ...
            return (
              <div
                key={index}
                className={`${
                  isOddSet && isLastItem ? "col-span-2 md:col-span-1" : "" // No último item ímpar, ocupa 2 colunas
                }`}
              >
                <IconGroupComponent
                  // key={index}
