...
export default function Section({ 
  ...
  return (
    <section className="bg-mintCream">
      ...
      <div className="pb-24 flex flex-col rounded-xl items-center justify-center space-y-4">
        {button?.map((btn, index) => (
          <div key={index} className="w-full flex justify-center">
            <a
              ...
              className={`w-max px-[22px] py-3 flex-none text-sans text-base font-semibold ${
                index === 0
                  ? 'bg-transparent border-[1px] border-sacramentoState rounded-xl text-sacramentoState'
                  : 'bg-sacramentoState rounded-xl text-secondary pl-4 pr-4 flex items-center'
              }`}
            >
              {btn.name}
