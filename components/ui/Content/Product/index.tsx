...
function Product({ icon, title, description, button, colorTheme }: JSX.IntrinsicElements["div"] & Props) {
  return (
    <>
      ...
      <div className="pt-4 pb-2">
        {title && (
          <h2 className={`font-sans text-[22px] font-medium ${setClass(colorTheme)} leading-tight-25 tracking-tight-2`}>{title}</h2>
        )}
      </div>
      <div className="pb-[32px] pr-[80px]">
        {description && (
          <p className={`text-base font-sans ${setClass(colorTheme)}-80 font-normal leading-tight-18`}>{description}</p>
        )}
      </div>
