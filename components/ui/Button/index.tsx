...
function setWidthDesktop(value) {
  switch (value) {
    case "Máximo (100%)":
      return "w-full";
...
function ButtonComponent(
  { text, link, icon, buttonTheme, tamanho, tamanhoDesktop }:
  ...
  return (
    <>
      <div className="flex items-center justify-center">
        <div
          className={`${setClass(buttonTheme)} ${setWidth(tamanho) || ""} lg:${
            setWidthDesktop(tamanhoDesktop) || ""
          } max-w-[706px] mx-4 md:mx-0 rounded-lg`}
        >
          <a
            href={link}
            className="flex items-center justify-center gap-3 rounded-[10px] text-base md:text-lg font-semibold text-center"
          >
            {text}
            {icon}
          </a>
